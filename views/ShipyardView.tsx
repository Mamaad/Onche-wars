...
import { LucideRocket, LucideInfo, LucideCrosshair, LucideShield, LucideBox, LucideLock } from 'lucide-react';
import { Ship, Resources, Building, Research } from '../types';
import { formatNumber, formatTime } from '../utils';
import { TechCard } from '../components/TechCard';
import { TechButton } from '../components/TechButton';

export const ShipyardView = ({ fleet, buildings, research, resources, onBuild }: { fleet: Ship[], buildings: Building[], research: Research[], resources: Resources, onBuild: (id: string, count: number) => void }) => {
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {fleet.map((s: Ship) => {
          ...
          return (
            <TechCard key={s.id} className="p-1 flex flex-col sm:flex-row h-full transition-all hover:border-slate-600">
              <div 
              ...
              >
                {s.image ? (
                  <img src={s.image} alt={s.name} className="absolute inset-0 w-full h-full object-cover opacity-80 transition-opacity duration-300" />
                ) : (
